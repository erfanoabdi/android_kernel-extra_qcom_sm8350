...
{
	int i;
	u32 int_en = readl_relaxed(PGD_THIS_EE(dev, PGD_PORT_INT_EN_EEn,
							dev->ver));
	/*
	...
	for (i = 0; i < dev->port_nums; i++) {
		...
		if (pstat & (1 << endpoint->port_b)) {
			u32 val = readl_relaxed(PGD_PORT(dev, PGD_PORT_STATn,
	...
	writel_relaxed((int_en & (~pstat)),
			PGD_THIS_EE(dev, PGD_PORT_INT_EN_EEn,
					dev->ver));
	/* clear port interrupts */
	writel_relaxed(pstat, PGD_THIS_EE(dev, PGD_PORT_INT_CL_EEn,
							dev->ver));
...
void msm_hw_set_port(struct msm_slim_ctrl *dev, u8 pipenum, u8 portnum)
{
	...
	writel_relaxed(set_cfg, PGD_PORT(dev, PGD_PORT_CFGn, pipenum,
								dev->ver));
	writel_relaxed(DEF_BLKSZ, PGD_PORT(dev, PGD_PORT_BLKn, pipenum,
								dev->ver));
	writel_relaxed(DEF_TRANSZ, PGD_PORT(dev, PGD_PORT_TRANn, pipenum,
								dev->ver));
	...
	if (chan->prot == SLIM_PUSH || chan->prot == SLIM_PULL) {
		...
		writel_relaxed(set_cfg, PGD_PORT(dev, PGD_PORT_PSHPLLn,
							pipenum, dev->ver));
	}
...
static void msm_slim_disconn_pipe_port(struct msm_slim_ctrl *dev, u8 pn)
{
	...
	u32 int_port = readl_relaxed(PGD_THIS_EE(dev, PGD_PORT_INT_EN_EEn,
					dev->ver));

	writel_relaxed(0, PGD_PORT(dev, PGD_PORT_CFGn, (endpoint->port_b),
					dev->ver));
	writel_relaxed((int_port & ~(1 << endpoint->port_b)),
		PGD_THIS_EE(dev, PGD_PORT_INT_EN_EEn, dev->ver));
...
int msm_slim_connect_pipe_port(struct msm_slim_ctrl *dev, u8 pn)
{
	...
	stat = readl_relaxed(PGD_PORT(dev, PGD_PORT_STATn, endpoint->port_b,
					dev->ver));
	if (dev->ctrl.ports[pn].flow == SLIM_SRC) {
			...
			u32 len, struct completion *comp)
{
	...
	if (!ret) {
		...
		u32 int_port = readl_relaxed(PGD_THIS_EE(dev,
				PGD_PORT_INT_EN_EEn, dev->ver));
